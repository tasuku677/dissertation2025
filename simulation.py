...
class TdlsFanetSimulation:
    ...
    def update_direct_trust(self, uav_x, current_time: int): #uav_x から見た隣接ノード郡 uav_j の直接信頼値を求める．
        """
        引数：評価者 uav_x,
    ...
    def update_final_trust(self, uav_j) -> float: 
        ...
        if not vals:
            avg_hybrid_by_target = (self.config.INITIAL_TRUST, self.config.INIT_SIGMA)
        else:
